...
##################################
    ...
    if ( $fileInfoRef->{'filepath'} =~ /reports\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/analysisResults\/(.*?\.vcf|.*?\.vcf.gz)$/ ) {
        $fileInfoRef->{'study'}                           = $1;
    ...
    elsif ( $fileInfoRef->{'filepath'} =~ /TGen\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?\.vcf|.*?\.vcf.gz)$/ ) {
        $fileInfoRef->{'study'}                           = $2;
    ...
    elsif ( $fileInfoRef->{'filepath'} =~ /reports\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?\.vcf|.*?\.vcf.gz)$/ ) {
        $fileInfoRef->{'study'}                           = $1;
        $fileInfoRef->{'patient'}                         = $2;
        $fileInfoRef->{'collectionDate'}                  = $3;
        $fileInfoRef->{'sampleSubgroupVisitSampleNumber'} = $4;
        ...
        if ( $fileInfoRef->{'filepath'} =~ /VCF38\/(.*?)\/(.*?)\/(.*?)\/(.*.vcf|.*.vcf.gz)$/ ) {
            $fileInfoRef->{'study'}    = $1;
            $fileInfoRef->{'patient'}  = $2;
        ...
        elsif ( $fileInfoRef->{'filepath'} =~ /VCF38\/(.*?)\/(.*?)\/(.*.vcf|.*.vcf.gz)$/ ) {
            $fileInfoRef->{'study'}    = $1;
            ...
            $fileInfoRef->{'filename'} = $1;
            if ( $fileInfoRef->{'filename'} =~ /.*\/(.*?)(\.vcf|\.vcf.gz)$/ ) {
                $fileInfoRef->{ $RunParameters->{'uidName'} } = $1;
                if ( $fileInfoRef->{ $RunParameters->{'uidName'} } =~ /^(.*?)_(.*?)_(.*?)_(.*?)_(.*?)/ ) {
                    $fileInfoRef->{'study'}   = $1;
                    $fileInfoRef->{'patient'} = "$1.$2";
