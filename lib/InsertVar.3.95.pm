...
sub processVcfFile {
    ...
    $RunParameters->{'filesCollection'}->update( { 'filepath' => $filepath }, $fileInfoRef );
...
#            @list = `find $RunParameters->{'vcfPath'} -name "*.vcf"`;
            if (exists ( $RunParameters->{'mtime'})) {
               @list = `find -L $RunParameters->{'vcfPath'} -iname "*.vcf" -o -iname "*.vcf.gz" -mtime $RunParameters->{'mtime'}`;
            }else{
                @list = `find -L $RunParameters->{'vcfPath'} -iname "*.vcf" -o -iname "*.vcf.gz"`;
           }
    ...
    if (exists($RunParameters->{'scrubEnabled'} ) ) { 
        if ($RunParameters->{'scrubEnabled'} eq "on" || $RunParameters->{'scrubEnabled'} eq "1" || $RunParameters->{'scrubEnabled'} eq "true") {
            ...
            while (my $doc=$cursor->next) {
                ...
                $RunParameters->{$variantCollectionDb}->update( {'_id'=>$doc->{'_id'}}, { '$pull' => { 'variants' => { 'filepath' => $mongoFilepath } } }, { 'safe' => 1  } );
            ...
            $RunParameters->{'uidCollection'}->update_many( {}, { '$pull' => { 'filepaths' => { 'filepath' => $mongoFilepath } } }, { 'safe' => 1 } );
...
sub openVcf {
    ...
    my $line = "";
    my $FILE;
    if ($fileInfoRef->{'filepath'} =~ /vcf.gz$/) {
        open( FILE, "gunzip -c $fileInfoRef->{'filepath'} |") or warn "Can't gunzip in openVCF $fileInfoRef->{'filepath'}: $!";
    }
    elsif ($fileInfoRef->{'filepath'} =~ /vcf$/) {
...
sub CreateOnPath {
    ...
    if ( exists( $RunParameters->{'CreateOnPath'} ) ) {
        ...
        for (my $i=0;$i<=$RunParameters->{'CollectionCount'};++$i) {
            ...
            my @list = ();
            if (exists ( $RunParameters->{'mtime'})) {
               @list = `find $RunParameters->{'vcfPath'} -iname "*.vcf" -o -iname "*.vcf.gz" -mtime $RunParameters->{'mtime'}`;
            }else{
                @list = `find $RunParameters->{'vcfPath'} -iname "*.vcf" -o -iname "*.vcf.gz"`;
           }
