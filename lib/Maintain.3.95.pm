...
sub closeConnection {
    ...
    $RunParameters->{'locksCollection'}->update( { 'bufferIndex' => int( $RunParameters->{'bufferIndex'}->{$in} ) }, { '$set' => { 'status' => int(1), 'in' => "None", "hostname" => "none", "pid" => "None" } } );
...
sub loadLocations {
	...
	if ( $fields[8] =~/gene_biotype "protein_coding"/ ) { next }
	my $chr = $fields[0];
